...
export default function BInfo() {
  ...
  const beforesubmitcheck = async () =>{
    if(!formData.shopName || !formData.coinValue || !formData.pin || !formData.localArea || !formData.tagline  ){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const responseOwnerId = await axios.post("http://localhost:3000/shop/id", { phone });
      const ownerId = responseOwnerId.data.message; // make sure `.data.message` is correct
  
      // Create payload with updated ownerId
      const payload = {
        ...formData,
        ownerId,
      };
  
      const response = await fetch('http://localhost:3000/shop/info', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload), // ✅ now contains ownerId
      });
...
/>
     ...
     id="coinValue"
     name="coinValue"
     placeholder='value must lie beween 5 to 10'
     max={10}
     min={5}
     step={1}
     onChange={handleChange}
